...
config_help:
	...
	@$(ECHO)
	@$(ECHO) "   To add to add to the existing configuration do:"
	...
	@$(ECHO) "       Build the GCS with OpenSceneGraph support, this enables the PFD"
	@$(ECHO) "       Options: 0 or 1"
	@$(ECHO)
	@$(ECHO) "   GCS_WITH_OSGEARTH=$(GCS_WITH_OSGEARTH)"
	@$(ECHO) "       Build the GCS with osgEarth support, this enables extra earth views"
	@$(ECHO) "       Options: 0 or 1"
	@$(ECHO)
...
# TODO: add other things like downloads and tools directoriesi, linux make install parameters
